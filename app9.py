...
def index():
    ...
    return render_template('index9.html', map=map_html, stop_names=stop_names, shapes_data=shapes_data, stops_data=stops_data)
   ...
   # ... in /search_routes
    ...
    for route_id, route_details in final_updated_routes_with_stops_data.items():
        for trip_id in route_details['trip_ids']:
            if trip_id in valid_real_time_trip_ids:
                route_name = route_details['route_long_name']
                eta = etas.get(trip_id)
                if eta is not None:
                    if route_name not in routes_with_etas or eta < routes_with_etas[route_name]:
                        routes_with_etas[route_name] = eta  # Update with the earliest ETA

    # Format the response to include route names and the earliest ETA for each route
    response = [{"route": route, "eta": f"{int(eta // 60)} min {int(eta % 60)} sec"} 
                for route, eta in routes_with_etas.items()]
